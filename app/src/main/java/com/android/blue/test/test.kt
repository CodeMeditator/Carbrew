...
fun main() {
    ...
    url =
        "https://m.ixigua.com/video/m/search/search_content/?aid=3586&keyword=$kw&device_id=&offset=$offset&count=10"
    Log.d(TAG, "url=$url")

    val headers = Headers.Builder().add("cookie", "ttwid=${MyApp.ttwid};ixigua-a-s=0;").build()
    ...
    if (json.startsWith("{") && json.endsWith("}")) {
        ...
        if ("success" == message) {
            ...
            for (i in 0 until length) {
                ...
                val detail_video_large_image =
                    video_detail_info?.optJSONObject("detail_video_large_image")
                ...
                val url = "https://m.ixigua.com/video/$groupId?title= " + URLEncoder.encode(
                    title, "UTF-8"
                ) + "&cover=" + URLEncoder.encode(
                    coverImageUrl, "UTF-8"
                ) + "&publishTime=$publishTime" + "&play_count=$playCount" + "&duration=$duration" + "&name=" + URLEncoder.encode(
                    userName, "UTF-8"
                ) + "&desc=" + URLEncoder.encode(desc, "UTF-8") + "&groupId=" + URLEncoder.encode(
                    groupId, "UTF-8"
                )

                data.add(MediaInfo2Data(
                    title, coverImageUrl!!, "", playCount.toString() + "次观看", userName
                ).apply {
                    spanSize = 8
